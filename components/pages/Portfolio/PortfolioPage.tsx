...
import PreLoading from "@/components/UI/Muscles/PreLoading/PreLoading";
...
const filters = useMemo(() => {
  ...
  return [ ...Array.from(new Set(serviceTitles))];
}, [services, locale]);
  ...
  const transformedProjects = useMemo(() => {
    ...
    const transformed = filteredProjects.map((project: ProjectItem) => {
      ...
      return {
        ...
        category:
          project.services?.[0]?.services_id?.title?.[locale as "en" | "ar"] ||
          project.technology_stack?.[0] ||
          "General",
        percentage: project.case_study_results?.[0]?.value || "+50%",
        description:
          project.case_study_results?.[0]?.description?.[
            locale as "en" | "ar"
          ] || "Project description",
